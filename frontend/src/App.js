...
// --- NEW Normalization Analyzer Component ---
function NormalizationAnalyzer() {
  ...
  // State for user-defined FDs
  const [functionalDependencies, setFunctionalDependencies] = useState([]); // [{id, determinants:[], dependents:[]}]

  // State for FD input form
  const [currentDeterminants, setCurrentDeterminants] = useState(new Set()); // Use Set for multi-select handling
  const [currentDependent, setCurrentDependent] = useState('');

  // Analysis results and loading states
  ...
  useEffect(() => {
      if (!selectedTable) {
          ...
          setCurrentDeterminants(new Set()); setCurrentDependent(''); setAnalysisResult(null); setError(null);
          return;
      }
      setIsLoadingColumns(true); setFunctionalDependencies([]); setAnalysisResult(null); setError(null);
  ...
  const addFD = () => {
      ...
      setFunctionalDependencies(prev => [
          ...prev,
          {
              ...
              dependents: [currentDependent] // Store dependent as array for potential future multi-dependent UI
          }
      ]);
      // Reset form
  ...
  const handleAnalyze = () => {
      ...
      const payload = {
          table: selectedTable,
          // Ensure dependents is always an array, even if UI only supports one now
      ...
      axios.post('http://localhost:5000/api/analyze_normalization', payload)
          ...
          .catch(err => {
               ...
               setError(err.response?.data?.error || err.message || "Normalization analysis failed.");
               setAnalysisResult(err.response?.data || null); // Show partial results if available on error
          })
              ...
              {/* Sticky Analyze Button */}
               <div className='sidebar-sticky-bottom'>
                  <button
                      ...
                      disabled={!selectedTable || isLoadingColumns || isLoadingAnalysis || !primaryKeyColumns.length} // Need PK
                  >
              ...
              {analysisResult && !error && (
                  <div>
                      {/* Display Keys */}
                       ...
                       {/* Display Analysis per Normal Form */}
