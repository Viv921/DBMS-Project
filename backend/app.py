...
def handle_schema():
    ...
    and attempts to create the corresponding database schema.
    """
    ...
    try:
        ...
        # Although we add FKs later, it's good practice to re-enable
        ...
        for fk_info in relationships_data:
            ...
            try:
                ...
                added_foreign_keys.append({
                 ...
                 # Attempt to rollback the ADD COLUMN if the constraint fails? Maybe too complex for now.
                 errors["fk_creation"].append({
                     "source_table": source_table_name,
                     "target_table": target_table_name,
                     "sql_add_col": sql_add_col,
                     "sql_add_fk": sql_add_fk,
                     "error": str(fk_error)
                 })
